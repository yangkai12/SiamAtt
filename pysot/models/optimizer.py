...
class DiMPSteepestDescentGN(nn.Module):
    ...
    def forward(self, weights, feat, bb, sample_weight=None, num_iter=None, compute_losses=True):
        ...
        filter_sz = (weights.shape[-2], weights.shape[-1]) # filter_sz:(4,4)
        output_sz = (feat.shape[-2] + (weights.shape[-2] + 1) % 2, feat.shape[-1] + (weights.shape[-1] + 1) % 2) # [19,19]

        # Get learnable scalars
        step_length_factor = torch.exp(self.log_step_length) # 0.7052
        reg_weight = (self.filter_reg*self.filter_reg).clamp(min=self.min_filter_reg**2)

        # Compute distance map
        dmap_offset = (torch.Tensor(filter_sz).to(bb.device) % 2) / 2.0  # bb为[288,288]的bounding box位置
        center = ((bb[..., :2] + bb[..., 2:] / 2) / self.feat_stride).reshape(-1, 2).flip((1,)) - dmap_offset
        dist_map = self.distance_map(center, output_sz) # [30,100,19,19]

        # Compute label map masks and weight    [30,100,19,19]-->[3,10,19,19]
        ...
        weight_iterates = [weights] # 初始滤波器权重
        losses = []
        ...
        for i in range(num_iter):
            ...
            # Compute residuals
            scores = filter_layer.apply_filter(feat, weights) # weights为Roi_pooling之后的特征  将分类层的特征与初始滤波器卷积
            ...
            if compute_losses:
                losses.append(((residuals**2).sum() + reg_weight * (weights**2).sum())/num_sequences)  # S1式
            ...
            # Update filter
            weights = weights - (step_length_factor * alpha.reshape(-1, 1, 1, 1)) * weights_grad # filter:weights
