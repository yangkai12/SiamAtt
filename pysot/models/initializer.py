...
class FilterPool(nn.Module):
    ...
    def forward(self, feat, bb):
        ...
        return self.prroi_pool(feat, roi1) # [30,512,18,18]-->[30,512,4,4]
...
class FilterInitializerLinear(nn.Module):
    ...
    def forward(self, feat, bb):
        ...
        num_images = feat.shape[0]  # [3,10,512,18,18]

        feat = self.filter_conv(feat.reshape(-1, feat.shape[-3], feat.shape[-2], feat.shape[-1]))  # 输入特征经过卷积层

        weights = self.filter_pool(feat, bb) # 得到初始权重[30,512,4,4]

        # If multiple input images, compute the initial filter as the average filter.
        if num_images > 1:  # 每个batchsize中的三张图片的权重取均值
            weights = torch.mean(weights.reshape(num_images, -1, weights.shape[-3], weights.shape[-2], weights.shape[-1]), dim=0)
