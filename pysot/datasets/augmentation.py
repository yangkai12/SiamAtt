...
class Augmentation:
    ...
    def __call__(self, image, bbox, size, data, gray=False):
        ...
        if data == 'search':
            ...
            atts_map, x_int, y_int, x_float, y_float = self.create_attention_mask(attentions, cfg.TRAIN.ratios, bbox) # image:[255,255,3]
        else:
            atts_map, x_int, y_int, x_float, y_float = [], [], [], [], []

        if x_int:
            tag_masks = np.ones((cfg.offs.max_objects), dtype=np.bool)
            ...
            tag_masks, tl_regrs, tl_tags = [], [], []

        return image, bbox, atts_map, tag_masks, tl_regrs, tl_tags
