...
class SiamRPNTracker(SiameseTracker):
    ...
    def track(self, img):
        ...
        x_crop = x_crop.squeeze(0)
        x_crop = x_crop.transpose(1, 0).transpose(2, 1).cpu().numpy()

        att_loc, att_score = self._convert_score_att(outputs['atts'], outputs['attentions']) # atts:目标特征图中心点位置

        cls_score = outputs['cls'][0][6].cpu()
        cls_score = cls_score.detach().numpy()
        plt.figure()
        plt.imshow(cls_score)
        plt.axis('off')
        plt.show()
        ...
        bbox = pred_bbox[:, best_idx] / scale_z  # pred_bbox:[4,3125] 对应于padding的目标位置
        lr = penalty[best_idx] * score[best_idx] * cfg.TRACK.LR

        cx = bbox[0] + self.center_pos[0]  # 中心
